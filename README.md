...
### Segment Selection
This folder contains various board configurations used during the MTURK expirements, as well as the script ```generate_segment_pairs.py``` which finds all segment pairs in our segment space for a given board confiugarion and segment length. These pairs are organized into categories from the first stage of data collection (see appendix D.3). The segment space is defined such that the x-axis is state value differences between the two segments and the y-axis is partial return differences between the two segments. There are four possible subsets: segment pairs that have the same start and end state, segment pairs that have the same start state and different end state, segment pairs that have different start states and the same end state, and segment pairs that have different start and end states. Using the ```generate_segment_pairs.py```, segment pairs are specified via their coordinates in the segment space as well as the desired subset from the list above.  
